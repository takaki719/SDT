...
# -------------------------------------------------------------------------
# Speech Disentanglement Encoder Components
# -------------------------------------------------------------------------
class Wav2vec2(torch.nn.Module):
    """MMS model for linguistic representation extraction"""
    def __init__(self, layer=7):  # Paper uses layer 7
        super().__init__() 
        # Using MMS (Massively Multilingual Speech) model as mentioned in the paper
...
class PitchEncoder(nn.Module):
    """Pitch encoder for F0 processing"""
    def __init__(self, in_channels=1, out_channels=256):
        super().__init__()
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size=1)
        
    def forward(self, f0):
        # f0: [B, 1, T] where T is 4x the mel-spectrogram time dimension
        # Apply log transformation as mentioned in the paper
        f0_log = torch.log(f0 + 1.0)
        return self.conv(f0_log)


class SourceEncoder(nn.Module):
    """Source encoder using WaveNet architecture"""
    def __init__(self, 
                 pitch_channels=256,
                 hidden_channels=256,
                 kernel_size=3,
    ...
    def forward(self, f0, espk, x_mask=None):
        ...
        # Downsample pitch to match content resolution
        e_f0 = F.avg_pool1d(e_f0, kernel_size=4, stride=4)
        
        # Process through WaveNet conditioned on speaker
...
class FilterEncoder(nn.Module):
    """Filter encoder using WaveNet architecture"""
    def __init__(self,
...
class SpeechDisentanglementEncoder(nn.Module):
    """Complete speech disentanglement encoder"""
    def __init__(self,
                 content_encoder,  # Wav2vec2
                 style_encoder,    # SynthesizerTrn for speaker
                 hidden_channels=256,
                 gin_channels=256):
        super().__init__()
    ...
    def forward(self, wav_input, f0, mel_input, lengths):
        # Extract content (linguistic) representation
...
# -------------------------------------------------------------------------
# Speech Diffusion Transformer
# -------------------------------------------------------------------------
class TimestepEmbedder(nn.Module):
    """Embed scalar diffusion timestep to a vector"""
...
class SpeechDiffusionTransformer(nn.Module):
    """Speech Diffusion Transformer for controllable voice style transfer"""
    def __init__(
        self,
        mel_size: tuple[int, int] = (80, 128),
    ...
    def _to_patches(self, x: Tensor) -> Tensor:
        """Convert mel-spectrogram to patches: [B, F, T] → [B, N, Pf*Pt]"""
        B, F, T = x.shape
    ...
    def _from_patches(self, tokens: Tensor) -> Tensor:
        """Convert patches back to mel-spectrogram: [B, N, Pf*Pt] → [B, F, T]"""
        B, N, _ = tokens.shape
    ...
    def forward(
        ...
        Forward pass of SDT
        Args:
            mel_noisy: Noisy mel-spectrogram [B, 80, 128]
            t: Timestep [B]
            z_src: Source embedding [B, 256]
            z_ftr: Filter embedding [B, 256] 
            espk: Speaker embedding [B, 256]
        Returns:
            Predicted noise [B, 80, 128]
        """
        # Embed timestep
...
class VPSDESchedule:
    ...
    def q_sample(self, x0: Tensor, t: Tensor, noise: Tensor) -> Tensor:
        """Forward diffusion: q(xt|x0) = N(xt; √ᾱt x0, (1-ᾱt)I)"""
    ...
    def predict_x0_from_eps(self, xt: Tensor, t: Tensor, eps: Tensor) -> Tensor:
        """Predict x0 from xt and noise prediction"""
        a_bar = self.alpha_bar(t).view(-1, 1, 1)
...
# -------------------------------------------------------------------------
# Complete Model with Loss
# -------------------------------------------------------------------------
class SDTVoiceConversion(nn.Module):
    """Complete SDT model for voice conversion"""
    def __init__(
        self,
        content_encoder,
        style_encoder,
        ...
        # Speech disentanglement encoder
    ...
    def compute_loss(self, mel_gt, wav_input, f0, lengths):
        """
        Compute diffusion loss with optional CFG dropout
        Args:
            mel_gt: Ground truth mel-spectrogram [B, 80, T]
            wav_input: Input waveform for content extraction
            f0: Fundamental frequency [B, 1, T*4]
            lengths: Sequence lengths
        """
        B = mel_gt.size(0)
        device = mel_gt.device
        
        # Encode speech components
        z_src, z_ftr, espk, w2v = self.encoder(wav_input, f0, mel_gt, lengths)
        
        # Take mean over time dimension for conditioning
        z_src_cond = z_src.mean(dim=2)  # [B, 256]
        z_ftr_cond = z_ftr.mean(dim=2)  # [B, 256]
        espk_cond = espk.squeeze(-1)    # [B, 256]
        
        # CFG dropout for source embedding
        ...
        # Reconstruction loss for encoder
    ...
    def voice_conversion(
        ...
        num_steps=6,
        guidance_scale=1.0,
        guidance_mode='source'  # 'source', 'null', or 'dual'
    ):
        """
        Perform voice conversion
        Args:
            source_wav: Source waveform
            source_f0: Source F0
            target_mel: Target mel-spectrogram for style
            source_lengths: Source sequence lengths
            target_lengths: Target sequence lengths
            num_steps: Number of diffusion steps
            guidance_scale: Scale for dual style guidance
            guidance_mode: Guidance mode for pitch control
        """
